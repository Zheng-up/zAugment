...
impl ApiServer {
    ...
    /// 启动 API 服务器
    pub async fn start(&mut self, state: Arc<crate::AppState>) -> Result<(), String> {
        ...
        let health_route = warp::path!("api" / "health")
            .and(warp::get())
            .and_then(move || health_handler(port));
...
async fn import_sessions_handler(
            ...
            match crate::add_token_from_session_internal(&session, &state.app_handle).await {
                Ok(response) => {
                    ...
                    if let Some(storage) = storage {
                        match storage.save_token(&token_data).await {
                            Ok(_) => ImportResult {
                                success: true,
                                token_data: Some(token_data),
                                error: None,
                        ...
                        ImportResult {
                            success: false,
                            token_data: None,
                            error: Some("Storage manager not initialized".to_string()),
                            session_preview: Some(mask_session(&session)),
                        }
                    }
                }
                Err(e) => ImportResult {
                    success: false,
                    token_data: None,
                    error: Some(format!("Failed to import session: {}", e)),
                    session_preview: Some(mask_session(&session)),
                },
            }
        });
    ...
    if request.detailed_response {
        ...
        let simple_result = SimpleImportResult {
            success: successful > 0,
            message: Some(format!("Imported {}/{} sessions successfully", successful, total)),
            error: if failed > 0 { Some(format!("{} sessions failed", failed)) } else { None },
            code: None,
        };
        Ok(warp::reply::with_status(
            warp::reply::json(&simple_result),
            warp::http::StatusCode::OK,
        ))
    }
}
...
fn with_state(
    state: Arc<crate::AppState>,
) -> impl Filter<Extract = (Arc<crate::AppState>,), Error = std::convert::Infallible> + Clone {
