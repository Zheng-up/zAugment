...
export default {
  ...
  methods: {
    async checkForUpdates(showNoUpdateMessage = false) {
      ...
      try {
        const response = await fetch('https://api.github.com/repos/Zheng-up/zAugment/releases/latest')
    ...
    getDownloadUrl(assets) {
      ...
      const platformAssets = {
        'windows': assets.find(asset => asset.name.includes('windows') && asset.name.endsWith('.exe')),
        'macos-intel': assets.find(asset => asset.name.includes('macos-x64') && asset.name.endsWith('.dmg')),
        'macos-arm': assets.find(asset => asset.name.includes('macos-arm64') && asset.name.endsWith('.dmg')),
        'linux': assets.find(asset => asset.name.includes('linux') && asset.name.endsWith('.AppImage'))
      }
    ...
    downloadUpdate() {
      if (this.downloadUrl) {
        ...
        window.open(`https://github.com/Zheng-up/zAugment/releases/tag/v${this.latestVersion}`, '_blank')
        this.closeUpdateModal()
      }
    },
