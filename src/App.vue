...
const computeHeaderCounts = (arr) => {
  ...
  return { active, banned, invalid, unknown };
};
...
const checkSingleAccountStatus = async (tokenId) => {
  ...
  try {
    ...
    showStatus(
      ...
      banStatus === "SUSPENDED" || banStatus === "INVALID_TOKEN"
        ? "error"
        : "success"
    );
...
const handleTokensImport = async (importedTokens) => {
  ...
  if (duplicateTokens.length > 0) {
    // 显示批量确认对话框
    const confirmMessage = `检测到 ${duplicateTokens.length} 个重复账号，是否全部覆盖更新？`;

    showDuplicateConfirm(
      null, // 批量操作不需要单个existing token
      ...
      async () => {
        // 取消覆盖：跳过重复账号
        errorCount += duplicateTokens.length;
  ...
  // 没有重复账号，直接完成导入
  await completeImport(successCount, updateCount, errorCount, emailFetchCount);
};
...
const completeImport = async (
  ...
  if (totalProcessed > 0) {
    ...
    try {
      ...
      const emailInfo =
        emailFetchCount > 0 ? `，自动获取到 ${emailFetchCount} 个邮箱` : "";
      const updateInfo = updateCount > 0 ? `，更新 ${updateCount} 个` : "";
      showStatus(
