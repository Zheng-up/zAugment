  ...
  <div class="app" @contextmenu.prevent>
    ...
    <div class="app-content">
            ...
            <div class="header-left">
              <div class="header-left-title">
                ...
                <div class="status-item saved">
                  <span class="status-count" v-if="savedTokensCount > 0">{{
                    savedTokensCount
                  }}</span>
                  <span class="status-label">正常</span>
                </div>
                <div class="status-item banned" v-if="bannedTokensCount > 0">
                  <span class="status-count">{{ bannedTokensCount }}</span>
                  <span class="status-label">封禁</span>
                </div>

                <div
                  class="status-item unchecked"
                  v-if="uncheckedTokensCount > 0"
                >
                  <span class="status-count">{{ uncheckedTokensCount }}</span>
                  <span class="status-label">未检测</span>
                </div>
              </div>
            </div>
            <div class="header-right">
      ...
      <div
        ...
        class="plugin-dialog-overlay"
        @click="showPluginDialog = false"
      >
        <div class="plugin-dialog" @click.stop>
          ...
          <div class="plugin-dialog-body">
            <p>
              为了更好地保护您的账户安全，请选择适合您的开发环境的防封插件：
            </p>
            <div class="plugin-options">
              <div class="plugin-option">
                <div class="plugin-option-icon vscode">
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                ...
                <div class="plugin-option-content">
                  <h4>VSCode 系列</h4>
                  <p>适用于 Visual Studio Code 编辑器</p>
                  <button @click="downloadVSCodePlugin" class="btn primary">
                    <svg
                      width="16"
                      height="16"
                      viewBox="0 0 24 24"
              ...
              <div class="plugin-option">
                <div class="plugin-option-icon jetbrains">
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                ...
                <div class="plugin-option-content">
                  <h4>JetBrains 系列</h4>
                  <p>适用于 IntelliJ IDEA、WebStorm 等</p>
                  <button @click="downloadJetBrainsPlugin" class="btn primary">
                    <svg
                      width="16"
                      height="16"
                      viewBox="0 0 24 24"
      ...
      >
        <p class="plugin-modal-description">
          为了更好地保护您的账户安全，请选择适合您的开发环境的防封插件：
        </p>
...
const headerCounts = ref({ active: 0, banned: 0, unknown: 0 });
...
const saveToken = async () => {
  try {
    ...
    showStatus("正在自动保存Token...", "info");

    // 自动保存到文件
    try {
      await saveTokensToFile(false);
      showStatus("Token已成功保存!", "success");
...
const checkSingleAccountStatus = async (tokenId) => {
  ...
  try {
    ...
    const statusText = result.is_banned ? "已封禁" : "正常";
    const emailText = emailUpdated ? "，已获取邮箱信息" : "";
    showStatus(
      `账号状态检测完成: ${statusText}${emailText}`,
      result.is_banned ? "error" : "success"
    );

    return { success: true, banned: result.is_banned };
  } catch (error) {
...
const refreshAllAccountData = async (manageState = true) => {
  ...
  try {
    ...
    if (results[0].status === "fulfilled") {
      ...
      statusResults.forEach((result, index) => {
        if (result.status === "fulfilled") {
          ...
          if (data.success) {
            ...
            console.log(
              `账号 ${index + 1} 检测成功: ${data.banned ? "已封禁" : "正常"}${
                data.emailUpdated ? ", 邮箱已更新" : ""
              }`
            );
          } else {
            failedCount++;
            console.warn(`账号 ${index + 1} 检测失败:`, data.error);
          }
        } else {
          failedCount++;
      ...
      console.error("状态检测整体失败:", results[0].reason);
      failedCount = tokens.value.length;
    }

    console.log(
      `刷新统计完成: 成功=${checkedCount}, 失败=${failedCount}, 封禁=${bannedCount}, 邮箱更新=${emailFetchCount}`
    );
    ...
    const failedInfo = failedCount > 0 ? `，${failedCount} 个账号检测失败` : "";

    if (bannedCount > 0) {
      showStatus(
        `刷新完成：发现 ${bannedCount} 个账号已被封禁，${checkedCount}/${totalAccounts} 个账号检测成功${emailInfo}${failedInfo}`,
        "warning"
      );
...
const openVSCodePlugin = async () => {
  showPluginHomeDialog.value = false;
  const url = "https://www.123684.com/s/f1TeTd-IMvHd?pwd=XmHG";
...
const openJetBrainsPlugin = async () => {
  showPluginHomeDialog.value = false;
  const url = "https://www.123684.com/s/f1TeTd-JMvHd?pwd=bUrn";
...
// 防封插件相关方法
const downloadVSCodePlugin = async () => {
  try {
    // 打开 VSCode 插件下载页面
    await invoke("open_url", {
      url: "https://www.123684.com/s/f1TeTd-xMvHd?pwd=Gsyf",
    });
    showStatus("正在打开 VSCode 插件下载页面...", "success");
    showPluginDialog.value = false;
  } catch (error) {
    console.error("打开 VSCode 插件页面失败:", error);
    showStatus("打开 VSCode 插件页面失败", "error");
  }
};

const downloadJetBrainsPlugin = async () => {
  try {
    // 打开 JetBrains 插件下载页面
    await invoke("open_url", {
      url: "https://www.123684.com/s/f1TeTd-4MvHd?pwd=lysa",
    });
    showStatus("正在打开 JetBrains 插件下载页面...", "success");
    showPluginDialog.value = false;
  } catch (error) {
    console.error("打开 JetBrains 插件页面失败:", error);
    showStatus("打开 JetBrains 插件页面失败", "error");
  }
};
  ...
  for (const tokenData of importedTokens) {
    try {
      ...
      if (tokenData.portal_url && !autoEmailNote) {
        try {
          showStatus(
            `正在自动获取第 ${successCount + 1} 个账号的邮箱信息...`,
            "info"
          );
      ...
      // 创建新的token对象
      const newToken = createNewToken(
        tokenData.tenant_url,
        tokenData.access_token,
        tokenData.portal_url || null,
        autoEmailNote
      );
  ...
  if (successCount > 0) {
    ...
    try {
      ...
      const emailInfo =
        emailFetchCount > 0 ? `，自动获取到 ${emailFetchCount} 个邮箱` : "";
      showStatus(
        `成功导入 ${successCount} 个账号${emailInfo}${
          errorCount > 0 ? `，${errorCount} 个失败或重复` : ""
        }`,
        "success"
      );
      ...
      // 导入完成后检测所有新账号状态
      setTimeout(() => {
        tokens.value.slice(-successCount).forEach((token) => {
          checkSingleAccountStatus(token.id);
        });
      }, 1000);
    } catch (error) {
      showStatus(
        `导入成功但自动保存失败，请手动保存。成功：${successCount}，失败：${errorCount}`,
        "warning"
      );
    }
  } else {
    showStatus("没有导入任何账号，请检查文件格式或是否存在重复账号", "warning");
  }
...
.status-item.saved .status-label::before {
  color: #22c55e;
}

.status-item.unsaved .status-label::before {
  color: #f59e0b;
}

.status-item.banned .status-label::before {
  color: #ef4444;
}

.status-item.unchecked .status-label::before {
  color: #9ca3af;
}
